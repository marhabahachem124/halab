...
# --- Database Setup (Shared with bot_logic.py) ---
...
def get_or_create_device_id():
    """
    Retrieves or creates a unique device ID for the current user's session.
    """
    # A unique session ID is used as a proxy for a unique device ID
    ...
    session = Session()
    try:
        # Check if this specific device ID already exists in the database
...
def get_bot_state():
    session = Session()
    try:
        return session.query(BotState).first()
    finally:
        session.close()

def update_bot_state_from_ui(**kwargs):
    session = Session()
    try:
...
def get_logs():
    session = Session()
    try:
        logs = session.query(BotLog).order_by(BotLog.timestamp.desc()).limit(100).all()
...
if not st.session_state.is_authenticated:
    ...
    if not bot_state:
        st.error("Could not retrieve bot state from database. Please check the bot_logic.py script and database connection.")
    else:
        ...
        if st.session_state.page == 'inputs':
            ...
            user_token = st.text_input("Enter your Deriv API token:", type="password", key="api_token_input", value=bot_state.user_token if bot_state.user_token else "")
            base_amount = st.number_input("Base Amount ($)", min_value=0.5, step=0.5, value=bot_state.base_amount)
            tp_target = st.number_input("Take Profit Target ($)", min_value=1.0, step=1.0, value=bot_state.tp_target if bot_state.tp_target else 1.0)
            max_consecutive_losses = st.number_input("Max Consecutive Losses", min_value=1, step=1, value=bot_state.max_consecutive_losses)
            ...
            with col1:
                if st.button("Start Bot", type="primary"):
                    if not user_token:
                        ...
                        update_bot_state_from_ui(is_running=True, user_token=user_token, base_amount=base_amount,
                                                 current_amount=base_amount, consecutive_losses=0, total_wins=0, total_losses=0,
                                                 tp_target=tp_target, max_consecutive_losses=max_consecutive_losses)
                        st.success("Bot started! You can close this tab.")
                        st.rerun()
            with col2:
                if st.button("Stop Bot"):
                    update_bot_state_from_ui(is_running=False)
                    st.warning("Bot will stop soon. You can close this tab.")
                    st.rerun()
        
        elif st.session_state.page == 'logs':
            st.header("2. Live Bot Logs")
            st.markdown(f"Wins: {bot_state.total_wins} | Losses: {bot_state.total_losses}")
            log_records = get_logs()
