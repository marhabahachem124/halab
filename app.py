...
def analyse_data(data):
    try:
        ...
        # --- شرط التأكيد العكسي (حركة آخر دقيقة) ---
        if len(data) >= 1:
            last_candle = data.iloc[-1]
            last_candle_is_up = last_candle['Close'] > last_candle['Open']
            
            # إذا كانت المؤشرات هبوط والشمعة الأخيرة صعود
            if (provisional_decision == "📉 هبوط" and last_candle_is_up):
                return "📈 صعود", None
            # إذا كانت المؤشرات صعود والشمعة الأخيرة هبوط
            elif (provisional_decision == "📈 صعود" and not last_candle_is_up):
                return "📉 هبوط", None
            else:
                # إذا كانت المؤشرات والشمعة في نفس الاتجاه، لا توجد صفقة
                st.info("⚠️ المؤشرات والشمعة الأخيرة في نفس الاتجاه. لا توجد صفقة.")
